...
scope Lieutenant 
	...
	function Lcqy_ZeroCast takes unit u, boolean b returns nothing
		...
		call SaveBoolean(DynamicData, GetHandleId(M_GetSpellAbilityUnit()), ZeroCast,  b)
	endfunction
	...
	function Crosskill takes nothing returns nothing
		local boolean b = LoadBoolean(DynamicData, GetHandleId(M_GetSpellAbilityUnit()) ,ZeroCast)
	...
	function Stomp takes nothing returns nothing
		local boolean isZeroCast = LoadBoolean(DynamicData, GetHandleId(M_GetSpellAbilityUnit()), ZeroCast)
		if isZeroCast then
			call Lcqy_ZeroCast(M_GetSpellAbilityUnit(), false) //移除零重施法状态
